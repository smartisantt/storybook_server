...
class LoginLog(BaseModle, models.Model):
    ...
    userAgent = models.CharField(max_length=1024, verbose_name='登录平台', null=True)
...
class AudioStory(BaseModle, models.Model):
    ...
    playTimes = models.IntegerField(default=0)  # 播放次数
...
class Behavior(BaseModle, models.Model):
    ...
    adminStatus = models.CharField(max_length=64, default="unCheck") # 人工审核状态unCheck check checkFail
...
class SystemNotification(BaseModle):
    ...
    publishState = models.IntegerField(default=0)
    # publishState 0未推送 1 极光发布成功 2 极光推送失败  3 修改成功 4 修改失败 5 删除成功 6 删除失败 7 增加成功 8 增加失败
    isDelete = models.BooleanField(default=False)
